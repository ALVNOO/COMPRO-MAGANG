...
.btn-dl:hover { background: #C41E1A; color: #fff; }
...
<x-dashboard.page-context-bar
    ...
    :role="$role === 'mentor' ? 'mentor' : 'peserta'"
/>
...
<div class="profile-layout">
    ...
    <div class="photo-card">
        <div class="photo-card-top">
            ...
            <span class="photo-role">
                @if($role === 'mentor') Pembimbing @else Peserta Magang @endif
            </span>
        </div>
    ...
    <div>
        ...
        @if($role === 'participant')
        {{-- ── PESERTA: Informasi Akun ── --}}
        <div class="info-card">
            <div class="card-head">
                <div class="card-head-icon"><i class="fas fa-user-circle"></i></div>
                <div>
                    <div class="card-head-title">Informasi Akun</div>
                    <div class="card-head-sub">Data biodata peserta magang</div>
                </div>
            </div>
            <div class="card-body">
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-user"></i></div>
                    <div><div class="ro-label">Nama Lengkap</div><div class="ro-value">{{ $user->name ?? '-' }}</div></div>
                </div>
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-envelope"></i></div>
                    <div><div class="ro-label">Email</div><div class="ro-value">{{ $user->email ?? '-' }}</div></div>
                </div>
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-graduation-cap"></i></div>
                    <div><div class="ro-label">NIM</div><div class="ro-value">{{ $user->nim ?? '-' }}</div></div>
                </div>
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-phone"></i></div>
                    <div><div class="ro-label">No. HP</div><div class="ro-value">{{ $user->phone ?? '-' }}</div></div>
                </div>
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-university"></i></div>
                    <div><div class="ro-label">Universitas</div><div class="ro-value">{{ $user->university ?? '-' }}</div></div>
                </div>
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-book"></i></div>
                    <div><div class="ro-label">Jurusan</div><div class="ro-value">{{ $user->major ?? '-' }}</div></div>
                </div>
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-id-card"></i></div>
                    <div><div class="ro-label">NIK (No. KTP)</div><div class="ro-value">{{ $user->ktp_number ?? '-' }}</div></div>
                </div>
            </div>
        </div>
        ...
        <div class="info-card">
            <div class="card-head">
                ...
                <div>
                    <div class="card-head-title">Status Pengajuan Magang</div>
                    <div class="card-head-sub">Informasi penempatan dan dokumen</div>
                </div>
            </div>
            <div class="card-body">
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-info-circle"></i></div>
                    <div>
                ...
                <div class="ro-row">
                    ...
                    <div>
                        <div class="ro-label">Divisi Penempatan</div>
                        <div class="ro-value">
                ...
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-user-tie"></i></div>
                    <div>
                        <div class="ro-label">Mentor</div>
                        <div class="ro-value">
                ...
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-tags"></i></div>
                    <div>
                        <div class="ro-label">Bidang Peminatan</div>
                        <div class="ro-value">{{ $application->fieldOfInterest->name ?? '-' }}</div>
                    </div>
                </div>
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-calendar"></i></div>
                    <div>
                        <div class="ro-label">Periode Magang</div>
                        <div class="ro-value">
                ...
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-file-pdf"></i></div>
                    <div>
                        <div class="ro-label">Surat Penerimaan</div>
                        <div class="ro-value">
                            @if($application->acceptance_letter_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($application->acceptance_letter_path))
                                <a href="{{ route('dashboard.acceptance-letter.download') }}" class="btn-dl" target="_blank"><i class="fas fa-download"></i> Download</a>
                            @else
                                <span style="color:#9CA3AF;font-style:italic;font-size:.82rem;">Belum tersedia</span>
                            @endif
                        </div>
                    </div>
                </div>
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-map-marked-alt"></i></div>
                    <div>
                        <div class="ro-label">Surat Izin Masuk Lokasi</div>
                        <div class="ro-value">
                            @if($application->location_permission_letter_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($application->location_permission_letter_path))
                                <a href="{{ asset('storage/' . $application->location_permission_letter_path) }}" class="btn-dl" target="_blank"><i class="fas fa-download"></i> Download</a>
                            @else
                                <span style="color:#9CA3AF;font-style:italic;font-size:.82rem;">Belum tersedia</span>
                            @endif
                        </div>
                ...
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-file-contract"></i></div>
                    <div>
                        <div class="ro-label">Pakta Integritas</div>
                        <div class="ro-value">
                            @if($application->integrity_pact_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($application->integrity_pact_path))
                                <a href="{{ asset('storage/' . $application->integrity_pact_path) }}" class="btn-dl" target="_blank"><i class="fas fa-download"></i> Download</a>
                            @else
                                <span style="color:#9CA3AF;font-style:italic;font-size:.82rem;">Belum tersedia</span>
                            @endif
                        </div>
                ...
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-sticky-note"></i></div>
                    <div>
                        <div class="ro-label">Catatan</div>
                        <div class="ro-value">{{ $application->notes }}</div>
                    </div>
                </div>
                @endif
            </div>
        ...
        <div class="info-card">
            ...
            <div class="card-body">
                ...
                <div class="ro-row">
                    ...
                    <div><div class="ro-label">NIK</div><div class="ro-value">{{ $user->username ?? '-' }}</div></div>
                </div>
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-phone"></i></div>
                    <div><div class="ro-label">No. HP</div><div class="ro-value">{{ $user->phone ?? '-' }}</div></div>
                ...
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-building"></i></div>
                    <div><div class="ro-label">Divisi</div><div class="ro-value">{{ $divisionMentor->division->division_name }}</div></div>
                </div>
                @endif
                <div class="ro-row">
                    <div class="ro-icon"><i class="fas fa-users"></i></div>
                    <div><div class="ro-label">Peserta Aktif</div><div class="ro-value">{{ $mentorParticipants->count() }} peserta</div></div>
                </div>
            </div>
        </div>
